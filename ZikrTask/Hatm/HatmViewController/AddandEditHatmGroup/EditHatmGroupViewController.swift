//
//  EditHatmGroupViewController.swift
//  ZikrTask
//
...
protocol EditHatimGroupDelegate: AnyObject {
    func editGroup()
}

class EditHatmGroupViewController: UIViewController {
    ...
    let addGroupButton: UIButton = {
        let button = UIButton()
        button.setTitle("O'zgartirish", for: .normal)
    ...
    weak var delegate: EditHatimGroupDelegate?
    var groupID: String?
    var isPublic: Bool = false
    
    private var editHatmGroupViewModel = EditHatmGroupViewModel()
    ...
    @objc func addGroupButtonTapped() {
        ...
        let request = HatmGroupEditRequest(
            name: name,
            groupType: "QURAN",
            isPublic: isPublic,
            guruhImg: "",
        ...
        editHatmGroupViewModel.editGroup(groupID: groupID, request: request) { result in
            switch result {
            case .success(let response):
                print("Group edited successfully: \(response)")
                self.delegate?.editGroup()
...
extension EditHatmGroupViewController: UITextFieldDelegate {
