//
//  GroupHatimCollectionViewCell.swift
//  ZikrTask
//
...
class GroupHatimCollectionViewCell: UICollectionViewCell {
    
    static let identifier = "GroupHatimCollectionViewCell"
    ...
    func updateProgress() {
        guard let text = hatimCountLabel.text else { return }
        
        let components = text.split(separator: "/")
        
        if components.count == 2,
           let currentCount = Int(components[0]),
           let totalCount = Int(components[1]) {
            
            let progress = Float(currentCount) / Float(totalCount)
