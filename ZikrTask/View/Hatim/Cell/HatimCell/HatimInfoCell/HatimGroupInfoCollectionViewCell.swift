...
class HatimGroupInfoCollectionViewCell: UICollectionViewCell {
    ...
    //    let selectionBlurView: UIVisualEffectView = {
    ...
    @objc func toggleDropdown() {
        ...
        UIView.animate(withDuration: 0.3) {
            if self.isExpanded {
                ...
                // Show buttons based on label text
                if self.label.text == self.startButton.currentTitle {
                    self.finishButton.isHidden = false
                    self.startButton.isHidden = true
                    
                    self.startButton.snp.removeConstraints()
                    
                    self.finishButton.snp.makeConstraints { make in
                        make.top.equalTo(self.label.snp.bottom).offset(1)
                ...
                } else if self.label.text == self.finishButton.currentTitle {
                    self.startButton.isHidden = false
                    self.finishButton.isHidden = true
                    
                    self.finishButton.snp.removeConstraints()
                ...
                self.arrowImage.image = UIImage(systemName: "chevron.up")
            } else {
