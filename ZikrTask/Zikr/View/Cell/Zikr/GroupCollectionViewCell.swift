...
class GroupCollectionViewCell: UICollectionViewCell {
    ...
    let personalZikrCountContainerView: UIView = {
        let view = UIView()
        view.backgroundColor = .darkMode
        view.layer.cornerRadius = 10
        view.layer.shadowColor = UIColor.black.cgColor
    ...
    let personalZikrCountLabel = CustomLabel(
        text: "",
        textColor: .white,
        fontSize: .systemFont(ofSize: 10),
        numberOfLines: 0
    ...
    func setupUI() {
        ...
        blurView.contentView.addSubview(personalZikrCountContainerView)
        personalZikrCountContainerView.snp.makeConstraints { make in
            make.top.equalTo(groupContainerView.snp.top)
            make.right.equalTo(groupContainerView.snp.left).offset(-8)
            make.width.equalTo(self.snp.width).multipliedBy(0.08)
            make.height.equalTo(groupContainerView.snp.height)
        }
        
        personalZikrCountContainerView.addSubview(personalZikrCountLabel)
        personalZikrCountLabel.backgroundColor = .darkMode
        personalZikrCountLabel.snp.makeConstraints { make in
            make.centerX.centerY.equalToSuperview()
        }
        
        blurView.contentView.addSubview(textContainerView)
        textContainerView.snp.makeConstraints { make in
            make.top.equalTo(groupContainerView.snp.bottom).offset(2)
            make.right.equalTo(groupContainerView.snp.right)
            make.width.equalTo(self.snp.width).multipliedBy(0.34)
    ...
    func configureCell(with group: Group, and zikr: Zikr?, userSpecificCount: Int) {
        groupNameLabel.text = group.groupName

        let overallCount = group.zikrProgress.first?.zikrCount ?? 0
        personalZikrCountLabel.text = "\(userSpecificCount)"

        zikrCountLabel.text = "Count: \(overallCount) / \(zikr?.goal ?? 0)"

        if let goal = zikr?.goal, goal > 0 {
            let progress = Float(userSpecificCount) / Float(goal)
